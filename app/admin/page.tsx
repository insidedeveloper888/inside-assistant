      ...
      <div className="mx-auto max-w-5xl space-y-8">
        ...
        <div className="rounded-xl border border-border bg-card p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-foreground">👥 Team Members <span className="text-[10px] text-muted-foreground/60 font-normal">({members.length} members, {larkUsers.length} Lark users loaded)</span></h2>
            <button
          ...
          {showAdd && (
            <div className="rounded-lg border border-border bg-muted/50 p-4 space-y-3">
              <div className="grid grid-cols-2 gap-2">
                <input value={newMember.displayName} onChange={(e) => setNewMember({ ...newMember, displayName: e.target.value })} placeholder="Name *" className="rounded-lg border border-border bg-muted px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground outline-none focus:border-primary" />
                <input value={newMember.email} onChange={(e) => setNewMember({ ...newMember, email: e.target.value })} placeholder="Email (for web login)" className="rounded-lg border border-border bg-muted px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground outline-none focus:border-primary" />
                <input value={newMember.phone} onChange={(e) => setNewMember({ ...newMember, phone: e.target.value })} placeholder="Phone (for WhatsApp AI)" className="rounded-lg border border-border bg-muted px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground outline-none focus:border-primary" />
                <select value={newMember.larkOpenId} onChange={(e) => { const u = larkUsers.find(l => l.openId === e.target.value); setNewMember({ ...newMember, larkOpenId: e.target.value, larkName: u?.name || "" }); }} className="rounded-lg border border-border bg-muted px-3 py-2 text-xs text-foreground outline-none focus:border-primary">
                  <option value="">Link Lark account</option>
                  {larkUsers.map((u) => <option key={u.openId} value={u.openId}>{u.name} ({u.email || u.enName})</option>)}
                </select>
              </div>
              <div className="flex items-center gap-2">
                <select value={newMember.role} onChange={(e) => setNewMember({ ...newMember, role: e.target.value })} className="rounded-lg border border-border bg-muted px-3 py-2 text-xs text-foreground outline-none">
                  ...
                  if (isEditing) {
                    return (
                      <tr key={m.user_id} className="border-b border-border/50">
                        <td className="py-2 pr-2"><input value={editData.display_name ?? m.display_name ?? ""} onChange={(e) => setEditData({ ...editData, display_name: e.target.value })} className="w-full rounded border border-border bg-muted px-2 py-1 text-xs text-foreground outline-none" /></td>
                        <td className="py-2 pr-2"><input value={editData.email ?? m.email ?? ""} onChange={(e) => setEditData({ ...editData, email: e.target.value })} className="w-full rounded border border-border bg-muted px-2 py-1 text-xs text-foreground outline-none" /></td>
                        <td className="py-2 pr-2"><input value={editData.phone ?? m.phone ?? ""} onChange={(e) => setEditData({ ...editData, phone: e.target.value })} placeholder="60162193255" className="w-full rounded border border-border bg-muted px-2 py-1 text-xs text-foreground outline-none" /></td>
                        <td className="py-2 pr-2">
                          <select value={editData.lark_open_id ?? m.lark_open_id ?? ""} onChange={(e) => { const u = larkUsers.find(l => l.openId === e.target.value); setEditData({ ...editData, lark_open_id: e.target.value || null, lark_name: u?.name || null }); }} className="w-full rounded border border-border bg-muted px-2 py-1 text-xs text-foreground outline-none">
                        ...
                        <td className="py-2 pr-2">
                          <select value={editData.role ?? m.role} onChange={(e) => setEditData({ ...editData, role: e.target.value })} className="rounded border border-border bg-muted px-2 py-1 text-xs text-foreground outline-none">
                            <option value="member">Member</option>
                            <option value="manager">Manager</option>
                            <option value="director">Director</option>
                          </select>
                        </td>
                        <td className="py-2 pr-2 text-[10px] text-muted-foreground">Save first</td>
                        <td className="py-2">
                          <div className="flex gap-1">
                            ...
                            <button onClick={() => setEditingId(null)} className="rounded bg-muted px-2 py-1 text-[10px] text-foreground/80">Cancel</button>
                          </div>
                        </td>
                      </tr>
                    );
                  ...
                  return (
                    <tr key={m.user_id} className="border-b border-border/50 hover:bg-muted/30">
                      <td className="py-2.5 pr-4">
                        <span className="font-medium text-foreground">{m.lark_name || m.display_name}</span>
                      </td>
                      <td className="py-2.5 pr-4 text-muted-foreground">{m.email || <span className="text-muted-foreground/60">—</span>}</td>
                      <td className="py-2.5 pr-4 font-mono text-muted-foreground">{m.phone || <span className="text-muted-foreground/60">—</span>}</td>
                      <td className="py-2.5 pr-4">
                        {m.lark_verified ? (
                      ...
                      <td className="py-2.5 pr-4">
                        {waEntry ? (
                          ...
                          <span className="text-[10px] text-muted-foreground/60">{m.phone ? "Not enabled" : "No phone"}</span>
                        )}
                      </td>
                      <td className="py-2.5">
                        <div className="flex gap-1">
                          <button
                            ...
                            className="rounded px-2 py-1 text-[10px] text-muted-foreground hover:bg-muted hover:text-foreground/80"
                          >
                            Edit
                          </button>
                          <button
